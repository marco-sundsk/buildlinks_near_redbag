...
impl RedBagContract {
    ...
    pub(crate) fn return_redbag(&mut self, pk: PublicKey, amount: u128) {
        ...
        let wrap_ci = rb.claim_info.pop();
        assert!(wrap_ci.is_some(), "No claim info found.");
        let ci = wrap_ci.unwrap();

        rb.remaining_balance += ci.amount;
        self.red_info.insert(&pk, &rb);

        let wrap_records = self.receiver_redbag.get(&ci.user);
        assert!(wrap_records.is_some(), "No receiver records found.");
        let mut receiver_record = wrap_records.unwrap();
        receiver_record.pop();
        self.receiver_redbag.insert(&ci.user, &receiver_record);
    }
